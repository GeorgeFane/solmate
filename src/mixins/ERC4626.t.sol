...
contract ERC4626Test {
    ...
    function setUp() public {
        underlying = ERC20(0x02444D214962eC73ab733bB00Ca98879efAAa73d);
        vault = new ArborVault(underlying, "Mock Token Vault", "vwTKN");
    }

    function invariantMetadata() public {
        require(vault.name() == "Mock Token Vault", 
        "vault name not equal");

        require(vault.symbol() == "vwTKN",
        "vault symbol not equal");

        require(vault.decimals() == 18, 
        "decimals not equal");
    }

    function testMetadata(string calldata name, string calldata symbol) public {
        ArborVault vlt = new ArborVault(underlying, name, symbol);
        require(vlt.name() == name, 
        "metadata name doesn't match");
        require(vlt.symbol() == symbol,
        "metadata symbol doesn't match");
        require(address(vlt.asset()) == address(underlying),
        "metadata addresses doesn't match");
    }
